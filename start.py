...
from utils import db_check_user, db_write_words, words_convert, db_write_last_time


@dp.message_handler(commands=['user_words'])
async def send_my_words(message: types.Message):
    db_check_user(message.from_user.id, message.from_user.username)
    
    args = message.get_args().replace('@', '')
    top = user_words_top(message.chat.id, message.from_user.username, args)
...
async def inline_libera(inline_query: InlineQuery):
    db_check_user(inline_query.from_user.id, inline_query.from_user.username)
...
async def echo(message: types.Message):
    ...
    db_check_user(message.from_user.id, message.from_user.username)
    db_write_last_time(message.from_user.id)
    db_write_words(message.from_user.id, message.chat.id, words_convert(message.text))
